/* pfindblurb.c :
            Modified version of the pfind code that has an option to spit out
	    the full correlation. Use verbosity level 4 for this.
	    (still to be written)

	      Part of the quantum key distribution software for finding the
...
--
 ...
 LOGGING & NOTIFICATION:
   ...
   -V level:     Verbosity level control. level is integer, and by default set
		 ...
		 3 : more text 
		 4 : spits out correlation function
...
int main (int argc, char *argv[]) {
    ...
    double * result_coarse, * result_fine;
...
/* 	printf("control point 14\n");
    ...
    result_coarse = (double *)malloc(sizeof(double) * zhs);
    result_fine = (double *)malloc(sizeof(double) * zhs);
    ...
    /* retreive full correlation, coarde part  */
    for (i=0; i<zhs; i++) {
	result_coarse[i]=f1[i][0]; /* get real value */
    }
    ...
    /* retreive full correlation, coarse part  */
    for (i=0; i<zhs; i++) {
	result_fine[i]=f1[i][0]; /* get real value */
    }
	...
	case 3:
	    ...
	    fprintf(loghandle,"counts from channel 1: %d, channel2: %d\n",
		    ecnt1,ecnt2);
	    break;
    case 4: /* spit out correlation functions */
	for (i=0; i<zhs; i++) {
	    fprintf(loghandle,"%f %f\n",result_coarse[i],result_fine[i]);
	} 
	break;
