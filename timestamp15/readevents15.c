/* code to receive timestamp data from the new test board via a serial
   ...
   SIGPIPE:   terminates the acquisition gracefully without further
              messages on stderr.

   NOTES:	The raw events generated by this file are 64 bits. 4 LSB contains the
			4 detector click flags. The first 45 bits or a raw event should 
			contain the timestamps in 2 nano-seconds resolution. That require 
			14 bits of 0s in between the end of timestamp bits and the beginning 
			of detector flags. readevents4.c had 14 bits. This version corrects 
			that using :
			((uint64_t)(tsraw &0xffffffe0)<<15) + (tsraw & 0xf);. 
	...
	if (FD_ISSET(fh,&fds)) {
		...
		case 1: /* consolidated output in binary mode - fast code */
		    ...
		    for (j=0; j<cnt/4; j++) {
				...
				tl = loffset 
					+ ((uint64_t)(tsraw &0xfffffff0)<<15) 
					+ (tsraw & 0xf); /*changed to 15 bit shift to correct offset problem*/
				if (swapoption) {
					tl2=tl<<32L;
					tl = (tl>>32L) | tl2;
				}
